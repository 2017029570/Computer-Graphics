...
def exp(rv):
    ...
    return np.array([[(np.cos(th)) + (uv[0]*uv[0]*(1-np.cos(th))), (uv[0]*uv[1]*(1-np.cos(th))) - (uv[2]*np.sin(th)), (uv[0]*uv[2]*(1-np.cos(th))) + (uv[1]*np.sin(th))],
                     [(uv[1]*uv[0]*(1-np.cos(th))) + (uv[2]*np.sin(th)), (np.cos(th))+(uv[1]*uv[1]*(1-np.cos(th))), (uv[1]*uv[2]*(1-np.cos(th))) - (uv[0]*np.sin(th))],
                     [(uv[2]*uv[0]*(1-np.cos(th))) - (uv[1]*np.sin(th)), (uv[2]*uv[1]*(1-np.cos(th))) + (uv[0]*np.sin(th)), (np.cos(th)) + (uv[2]*uv[2]*(1-np.cos(th)))]])
...
def log(R):
    ...
    arr = normalized(np.array([v1, v2, v3])) * th
    return arr
...
def main():
    ...
    if not glfw.init():
        return
    window = glfw.create_window(480,480,'2017029570-10-1', None,None)
    if not window:
        glfw.terminate()
        return
